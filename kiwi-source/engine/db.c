...
int db_add(DB* self, Variant* key, Variant* value)
{
    pthread_mutex_lock(&self->lock);
        self->writers++;
        while(self->reading || self->writing) pthread_cond_wait(&self->cond, &self->lock);
        self->writing++;
    ...
    int mem_ret = memtable_add(self->memtable, key, value);
...
{
    pthread_mutex_lock(&self->lock);
        while(self->writers) pthread_cond_wait(&self->cond, &self->lock);
        self->reading++;
    pthread_mutex_unlock(&self->lock);

    int sst_ret;
    if (memtable_get(self->memtable->list, key, value) == 1) sst_ret = 1;
    else sst_ret = sst_get(self->sst, key, value);
